...
class Search extends DAO
{
  ...
  private function _makeSQLPremium($num = 2)
  {
    ...
    if ($this->withPattern ) {
      ...
      if(osc_search_pattern_method() == '') {
        ...
        $this->dao->where(sprintf("MATCH(d.s_title, d.s_description) AGAINST('%s' IN NATURAL LANGUAGE MODE)", $this->sPattern) );
      } else if(osc_search_pattern_method() == 'like') {
        $this->dao->where(sprintf("concat(d.s_title, d.s_description) like '%%%s%%'", $this->sPattern) );
      }
  ...
  private function _makeSQL($count = false,$premium = false)
  {
    ...
    if($this->withItemId) {
      ...
      if($count) {
        //$this->dao->select(DB_TABLE_PREFIX.'t_item.pk_i_id');  // update 420
        //$this->dao->select($extraFields); // plugins!          // update 420
        $this->dao->select('count('.DB_TABLE_PREFIX.'t_item.pk_i_id) as count');
      } else {
        $this->dao->select(DB_TABLE_PREFIX.'t_item.*, '.DB_TABLE_PREFIX.'t_item.s_contact_name as s_user_name');
      ...
      // ---------------------------------------------------------
      // groupBy
      if($this->groupBy != '' && $count !== true) {
        $this->dao->groupBy( $this->groupBy );
      }
      // having
      if($this->having != '' && $count !== true) {
