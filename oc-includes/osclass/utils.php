...
function osc_mailBeauty($text, $params) {
  ...
  $rwords = array(
    ...
    osc_get_ip() // Params::getServerParam('REMOTE_ADDR')
  );
...
function osc_check_plugin_update($product_key, $version = null)
{
  ...
  if ($uri != false) {
    return _need_update($uri, $version, $product_key);
  }
...
function _get_market_url($type, $product_key = '')
{
  ...
  if (in_array($type, array('plugins', 'themes', 'check_version', 'download', 'products_version'))) {
 ...
 * @return bool
 */
function _need_update($uri, $version, $product_key = '') {
  $data = osc_get_preference('market_products_version', 'osclass');
  $prepare = json_decode($data, true);

  if(isset($prepare['date']) && strtotime('-1 day') < strtotime($prepare['date']) && @$prepare['data'] <> '') {
...
function osc_do_upgrade()
{
   ...
   **** DOWNLOAD FILE ****
   ***********************/
  $data    = osc_file_get_contents('https://osclass-classifieds.com/api/latest_version.php?action=download');
...
{
  $data = osc_file_get_contents('https://osclass-classifieds.com/api/latest_version.php');
    ...
    if(@$data['type'] <> '' && (isset($data['download']) || isset($data['url']))) {
      ...
      if($data['type'] == 'theme') {
        if($data['theme_type'] == 'oc-admin') {
          $folder = ABS_PATH . 'oc-admin/themes/';
        } else {
          $folder = ABS_PATH . 'oc-content/themes/';
        }
      } else if($data['type'] == 'plugin') {
        $folder = ABS_PATH . 'oc-content/plugins/';
        ...
        $folder = ABS_PATH . 'oc-content/languages/';
      }
